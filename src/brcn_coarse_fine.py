...
class BiRecConvNet(object):
    ...
    def build_net(self, model):
        ...
        z = tensor.TensorType(dtype=config.floatX, broadcastable=(False,) * 5)(name='z')

        # forward net
        layers_f, params_f = self._init_layer(options['filter_shape'], options['rec_filter_size'])
        proj_f, mid_f, use_noise_f = self._build_model(x, options, layers_f, params_f, go_backwards=False)

        # backward net
        layers_b, params_b = self._init_layer(options['filter_shape'], options['rec_filter_size'])
        proj_b, mid_b, use_noise_b = self._build_model(x, options, layers_b, params_b, go_backwards=True)
        ...
        params_fin = dict()
        for k, v in params.iteritems():
            if '_3' in k or '_4' in k or '_5' in k:
                params_fin[k] = v

        proj = proj_f + proj_b[::-1]
        mid = mid_f + mid_b[::-1]
        # weight_decay = theano.shared(numpy_floatX(0.), borrow=True)
        # weight_decay_mid = theano.shared(numpy_floatX(0.), borrow=True)

        # for v in params.itervalues():
        #    weight_decay += (v ** 2).sum()


        # for v in params_mid.itervalues():
        #    weight_decay_mid += (v ** 2).sum()

        cost = ((y - proj) ** 2).sum() / (x.shape[1] * x.shape[0])
        mid_cost = ((z - mid) ** 2).sum() / (x.shape[1] * x.shape[0])

        f_x = theano.function([x], proj, name='f_proj')
        f_x_mid = theano.function([x], mid, name='f_mid')

        return x, y, z, f_x, f_x_mid, cost, mid_cost, params_fin, params_mid, params, use_noise_f, use_noise_b
    ...
    def _init_layer(self, filter_shape, rec_filter_size):
        ...
        for i in range(len(filter_shape)):
            ...
            params['b_' + str(i)] = theano.shared(np.zeros(filter_shape[i][0]).astype(config.floatX),
                                                  name='b_' + str(i), borrow=True)
    ...
    def _build_model(self, input, options, layers, params, go_backwards=False):
        ...
        def _step(x_, t_, r_, layer_):
            ...
            if int(layer_) is not 2 and int(layer_) is not 5:
        ...
        for i in range(len(options['filter_shape'])):
            ...
            rval, _ = theano.scan(_step, sequences=[rval],
                                  outputs_info=[tensor.alloc(numpy_floatX(0.), shape[0], shape[1], shape[2], shape[3]),
                                                tensor.alloc(numpy_floatX(0.), shape[0],
                                                             theano.shared(options['filter_shape'][i][0]),
                                                             shape[2] - padding, shape[3] - padding)],
                                  non_sequences=[i],
...
def train_brnn(
        ...
        saveto='14_seq_41085_yuv_scala_4_frm10_blur_2_c',  # The best model will be saved there
        model_path='14_seq_41085_yuv_scala_4_frm10_blur_2_c.npy',  # The model path
    ...
    print('... Loading data')
    train_path_x = '../data/14_seq_41085_yuv_scala_4_frm10_blur_2.mat'
    train_path_z = '../data/14_seq_41085_yuv_scala_2_frm10_blur_2.mat'
    ...
    train_set_x, train_set_y = load_data(train_path_x)
    train_set_z, _ = load_data(train_path_z)
    ...
    (x, y, z, f_x, f_x_mid, cost, cost_mid, params_fin, params_mid, params, use_noise_f, use_noise_b) = net.build_net(
        model)

    f_cost_mid = theano.function([x, z], cost_mid, name='f_cost_mid')
    lparams_mid = list(params_mid.values())
    grads_mid = tensor.grad(cost_mid, wrt=lparams_mid)
    f_grad_mid = theano.function([x, z], grads_mid, name='f_grad_mid')

    f_cost = theano.function([x, y], cost, name='f_cost')
    lparams = list(params_fin.values())
    ...
    f_grad_shared_mid, f_update_mid = optimizer(lr, params_mid, grads_mid, x, z, cost_mid)
    f_grad_shared, f_update = optimizer(lr, params_fin, grads, x, y, cost)
    ...
    try:
        for eidx in range(max_epochs):
            ...
            #                z = np.asarray([train_set_z[t, :, :, 3:-3, 3:-3]for t in train_index])


            #                x = theano.shared(value=x, borrow=True).dimshuffle(1, 0, 2, 3, 4).eval()
            #                z = theano.shared(value=z, borrow=True).dimshuffle(1, 0, 2, 3, 4).eval()

            #                cost = f_grad_shared_mid(x, z)
            #                f_update_mid(lrate)
            #                if np.mod(uidx_, dispFreq) == 0:
            #                    print('Epoch ', eidx, 'Update ', uidx_, 'Cost_x ', cost)
